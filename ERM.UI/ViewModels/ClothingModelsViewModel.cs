...
namespace ERM.UI.ViewModels
{
    public class ClothingModelsViewModel : ViewModelBase
    {
        ...
        private async Task AddAsync()
        {
            ...
            var (isSuccess, _) = await ExecuteSafeAsync(() => _service.CreateAsync(r.Name, r.Article, 
        ...
        private async Task EditAsync(ClothingModelDto? dto)
        {
            ...
            var (isSuccess, _) = await ExecuteSafeAsync(() => _service.EditAsync(
                r.ModelId, r.Name, r.Article, decimal.Parse(r.SewingPriceText), decimal.Parse(r.IroningPriceText), r.Description));
            if (!isSuccess) return;
